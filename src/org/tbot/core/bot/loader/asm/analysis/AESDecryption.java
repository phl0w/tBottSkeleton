...
public class AESDecryption {
    ...
    public AESDecryption(String ParmA, String ParmB, String ParmC) {
        this.ParmA = ParmA;
        this.ParmB = ParmB;
        this.ParmC = ParmC;
    }

    public byte[] decrypt(String Parm) throws Exception {
        try {
            i = Parm.length();
            if (0 == i) {
                return new byte[0];
        ...
        Byte = new byte[DecCases(Parm)];
        decrypt2(Byte, 0, Parm, (byte) -75);
        return Byte;
    }

    public int DecCases(String Parm) {
        int k;
        case0:
        {
            label1:
    ...
    public HashMap<String, byte[]> DecryptPack() {
        int dummy = 0;
        HashMap<String, byte[]> classes = new HashMap<String, byte[]>();
        try {
            byte[] key = decrypt(ParmA);
            byte[] ivParameterSpec = decrypt(ParmB);
            Cipher cipherObject;
            SecretKeySpec secretKeySpec = new SecretKeySpec(key, "AES");
            try {
                cipherObject = Cipher.getInstance("AES/CBC/PKCS5Padding");
                (cipherObject).init(2, secretKeySpec, new IvParameterSpec(
                        ivParameterSpec));
            ...
            InputStream packStream = new FileInputStream(new File(
                    ParmC));
